...
{
  echo "# verify ${M} (rollout diagnostics automation expected-effect projections)"
...
req=(
  ...
  "scripts/run-m67-rollout-diagnostics-expected-effect-scenario.mjs"
  "fixtures/vault/m67_scenario.json"
  "fixtures/vault/m67_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m67-rollout-diagnostics-expected-effect-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_expected_effect_output.json" "$LATEST_DIR/rollout_diagnostics_expected_effect_output.json"
