...
{
  echo "# verify ${M} (rollout diagnostics export + operator runbook hooks)"
  ...
  echo "$ AUTHZ_ENFORCE=1 SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE=1 PARTNER_PROGRAM_ADMIN_ALLOWLIST=ops-admin PARTNER_PROGRAM_ROLLOUT_POLICY_FREEZE_EXPORT_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m61-rollout-diagnostics-scenario.mjs"
...
req=(
  ...
  "scripts/run-m61-rollout-diagnostics-scenario.mjs"
  "fixtures/vault/m61_scenario.json"
  "fixtures/vault/m61_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m61-rollout-diagnostics-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_output.json" "$LATEST_DIR/rollout_diagnostics_output.json"
