...
{
  echo "# verify ${M} (freeze export overlay + rollout observability hardening)"
  ...
  echo "$ AUTHZ_ENFORCE=1 SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE=1 PARTNER_PROGRAM_ADMIN_ALLOWLIST=ops-admin PARTNER_PROGRAM_ROLLOUT_POLICY_FREEZE_EXPORT_ENFORCE=0 OUT_DIR=$OUT_DIR node scripts/run-m60-rollout-overlay-observability-scenario.mjs"
...
req=(
  ...
  "scripts/run-m60-rollout-overlay-observability-scenario.mjs"
  "fixtures/vault/m60_scenario.json"
  "fixtures/vault/m60_expected.json"
...
PARTNER_PROGRAM_ROLLOUT_POLICY_FREEZE_EXPORT_ENFORCE=0 \
OUT_DIR="$OUT_DIR" \
node scripts/run-m60-rollout-overlay-observability-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_overlay_observability_output.json" "$LATEST_DIR/rollout_overlay_observability_output.json"
