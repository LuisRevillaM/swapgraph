...
{
  echo "# verify ${M} (rollout diagnostics lifecycle signals + stale-control alerts)"
...
req=(
  ...
  "scripts/run-m63-rollout-diagnostics-lifecycle-scenario.mjs"
  "fixtures/vault/m63_scenario.json"
  "fixtures/vault/m63_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m63-rollout-diagnostics-lifecycle-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_lifecycle_output.json" "$LATEST_DIR/rollout_diagnostics_lifecycle_output.json"
