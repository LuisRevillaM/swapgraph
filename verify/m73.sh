...
{
  echo "# verify ${M} (rollout diagnostics automation execution journal anchors)"
...
req=(
  ...
  "scripts/run-m73-rollout-diagnostics-execution-journal-scenario.mjs"
  "fixtures/rollout/m73_scenario.json"
  "fixtures/rollout/m73_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m73-rollout-diagnostics-execution-journal-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_execution_journal_output.json" "$LATEST_DIR/rollout_diagnostics_execution_journal_output.json"
