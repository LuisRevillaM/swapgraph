...
{
  echo "# verify ${M} (rollout diagnostics continuity + checkpoint retention hardening)"
...
req=(
  ...
  "scripts/run-m62-rollout-diagnostics-continuity-scenario.mjs"
  "fixtures/vault/m62_scenario.json"
  "fixtures/vault/m62_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m62-rollout-diagnostics-continuity-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_continuity_output.json" "$LATEST_DIR/rollout_diagnostics_continuity_output.json"
