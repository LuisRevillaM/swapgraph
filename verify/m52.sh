...
{
  echo "# verify ${M} (vault reconciliation export surface)"
  ...
  echo "$ AUTHZ_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m52-vault-reconciliation-export-scenario.mjs"
...
req=(
  ...
  "docs/spec/examples/api/settlement.vault_reconciliation.export.response.json"

  "scripts/run-m52-vault-reconciliation-export-scenario.mjs"
  "fixtures/vault/m52_scenario.json"
  "fixtures/vault/m52_expected.json"
...
AUTHZ_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m52-vault-reconciliation-export-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/vault_reconciliation_export_output.json" "$LATEST_DIR/vault_reconciliation_export_output.json"
