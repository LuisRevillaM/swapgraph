...
{
  echo "# verify ${M} (steam vault live proof, integration-gated)"
  ...
  echo "$ AUTHZ_ENFORCE=1 INTEGRATION_ENABLED=1 OUT_DIR=$OUT_DIR node scripts/run-m86-steam-vault-live-proof-scenario.mjs"
...
req=(
  ...
  "docs/spec/examples/api/adapter.steam_tier1.live_proof.vault.record.request.json"
  "docs/spec/examples/api/adapter.steam_tier1.live_proof.vault.record.response.json"

  "docs/spec/schemas/SteamTier1VaultLiveProofRecordRequest.schema.json"
  "docs/spec/schemas/SteamTier1VaultLiveProofRecordResponse.schema.json"

  "scripts/run-m86-steam-vault-live-proof-scenario.mjs"
  "fixtures/integration/m86_scenario.json"
  "fixtures/integration/m86_expected.json"
...
AUTHZ_ENFORCE=1 INTEGRATION_ENABLED=1 OUT_DIR="$OUT_DIR" node scripts/run-m86-steam-vault-live-proof-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/steam_vault_live_proof_output.json" "$LATEST_DIR/steam_vault_live_proof_output.json"
