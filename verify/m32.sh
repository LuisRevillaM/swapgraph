...
{
  echo "# verify ${M} (agent delegation grants + TradingPolicy enforcement for SwapIntents)"
  ...
  echo "$ AUTHZ_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m32-delegation-scenario.mjs"
...
req=(
  ...
  "fixtures/delegation/m32_scenario.json"
  "fixtures/delegation/m32_expected.json"

  "scripts/run-m32-delegation-scenario.mjs"
...
AUTHZ_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m32-delegation-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/delegation_output.json" "$LATEST_DIR/delegation_output.json"
