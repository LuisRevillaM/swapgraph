...
{
  echo "# verify ${M} (rollout diagnostics automation attestation consistency verification)"
...
req=(
  ...
  "scripts/run-m69-rollout-diagnostics-attestation-consistency-scenario.mjs"
  "fixtures/vault/m69_scenario.json"
  "fixtures/vault/m69_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m69-rollout-diagnostics-attestation-consistency-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_attestation_consistency_output.json" "$LATEST_DIR/rollout_diagnostics_attestation_consistency_output.json"
