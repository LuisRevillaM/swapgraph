...
{
  echo "# verify ${M} (webhook reliability and dead-letter replay hardening)"
  ...
  echo "$ AUTHZ_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m81-webhook-reliability-scenario.mjs"
...
req=(
  ...
  "scripts/run-m81-webhook-reliability-scenario.mjs"
  "fixtures/commercial/m81_scenario.json"
  "fixtures/commercial/m81_expected.json"
  "milestones/M81.yaml"
  "verify/m81.sh"
)
...
AUTHZ_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m81-webhook-reliability-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/webhook_reliability_output.json" "$LATEST_DIR/webhook_reliability_output.json"
