...
{
  echo "# verify ${M} (checkpoint retention + expiry hardening)"
  ...
  echo "$ AUTHZ_ENFORCE=1 POLICY_AUDIT_EXPORT_CHECKPOINT_ENFORCE=1 POLICY_AUDIT_EXPORT_CHECKPOINT_RETENTION_DAYS=1 POLICY_AUDIT_RETENTION_DAYS=30 OUT_DIR=$OUT_DIR node scripts/run-m45-export-checkpoint-retention-scenario.mjs"
...
req=(
  ...
  "scripts/run-m45-export-checkpoint-retention-scenario.mjs"
  "fixtures/delegation/m45_scenario.json"
  "fixtures/delegation/m45_expected.json"
...
AUTHZ_ENFORCE=1 POLICY_AUDIT_EXPORT_CHECKPOINT_ENFORCE=1 POLICY_AUDIT_EXPORT_CHECKPOINT_RETENTION_DAYS=1 POLICY_AUDIT_RETENTION_DAYS=30 OUT_DIR="$OUT_DIR" node scripts/run-m45-export-checkpoint-retention-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/export_checkpoint_retention_output.json" "$LATEST_DIR/export_checkpoint_retention_output.json"
