...
{
  echo "# verify ${M} (rollout diagnostics automation execution receipt contract)"
...
req=(
  ...
  "scripts/run-m72-rollout-diagnostics-execution-receipt-scenario.mjs"
  "fixtures/rollout/m72_scenario.json"
  "fixtures/rollout/m72_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m72-rollout-diagnostics-execution-receipt-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_execution_receipt_output.json" "$LATEST_DIR/rollout_diagnostics_execution_receipt_output.json"
