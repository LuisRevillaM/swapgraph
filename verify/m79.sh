...
{
  echo "# verify ${M} (partner dashboard summary api contract)"
  ...
  echo "$ AUTHZ_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m79-dashboard-summary-scenario.mjs"
...
req=(
  ...
  "scripts/run-m79-dashboard-summary-scenario.mjs"
  "fixtures/commercial/m79_scenario.json"
  "fixtures/commercial/m79_expected.json"
  "milestones/M79.yaml"
  "verify/m79.sh"
)
...
AUTHZ_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m79-dashboard-summary-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/dashboard_summary_output.json" "$LATEST_DIR/dashboard_summary_output.json"
