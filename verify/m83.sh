...
{
  echo "# verify ${M} (dispute workflow and evidence bundle contracts)"
  ...
  echo "$ AUTHZ_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m83-dispute-evidence-bundle-scenario.mjs"
...
req=(
  ...
  "scripts/run-m83-dispute-evidence-bundle-scenario.mjs"
  "fixtures/commercial/m83_scenario.json"
  "fixtures/commercial/m83_expected.json"
  "milestones/M83.yaml"
  "verify/m83.sh"
)
...
AUTHZ_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m83-dispute-evidence-bundle-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/dispute_evidence_bundle_output.json" "$LATEST_DIR/dispute_evidence_bundle_output.json"
