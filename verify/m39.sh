...
{
  echo "# verify ${M} (consent tier hardening + delegated policy audit read contract)"
  ...
  echo "$ AUTHZ_ENFORCE=1 POLICY_CONSENT_TIER_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m39-consent-tier-audit-read-scenario.mjs"
...
req=(
  ...
  "scripts/run-m39-consent-tier-audit-read-scenario.mjs"
  "fixtures/delegation/m39_scenario.json"
  "fixtures/delegation/m39_expected.json"
...
AUTHZ_ENFORCE=1 POLICY_CONSENT_TIER_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m39-consent-tier-audit-read-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/consent_tier_audit_read_output.json" "$LATEST_DIR/consent_tier_audit_read_output.json"
