...
{
  echo "# verify ${M} (rollout diagnostics automation action-request templates)"
...
req=(
  ...
  "scripts/run-m65-rollout-diagnostics-automation-requests-scenario.mjs"
  "fixtures/vault/m65_scenario.json"
  "fixtures/vault/m65_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m65-rollout-diagnostics-automation-requests-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_automation_requests_output.json" "$LATEST_DIR/rollout_diagnostics_automation_requests_output.json"
