...
{
  echo "# verify ${M} (delegation lifecycle + agent read access expansion)"
  ...
  echo "$ AUTHZ_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m33-delegation-lifecycle-scenario.mjs"
...
req=(
  ...
  "fixtures/delegation/m33_scenario.json"
  "fixtures/delegation/m33_expected.json"

  "scripts/run-m33-delegation-lifecycle-scenario.mjs"
...
AUTHZ_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m33-delegation-lifecycle-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/delegation_lifecycle_output.json" "$LATEST_DIR/delegation_lifecycle_output.json"
