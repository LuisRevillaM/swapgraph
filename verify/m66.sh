...
{
  echo "# verify ${M} (rollout diagnostics automation plan-hash integrity anchors)"
...
req=(
  ...
  "scripts/run-m66-rollout-diagnostics-plan-hash-scenario.mjs"
  "fixtures/vault/m66_scenario.json"
  "fixtures/vault/m66_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m66-rollout-diagnostics-plan-hash-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_plan_hash_output.json" "$LATEST_DIR/rollout_diagnostics_plan_hash_output.json"
