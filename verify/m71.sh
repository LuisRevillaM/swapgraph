...
{
  echo "# verify ${M} (rollout diagnostics automation continuity-window hardening)"
...
req=(
  ...
  "scripts/run-m71-rollout-diagnostics-continuity-window-scenario.mjs"
  "fixtures/vault/m71_scenario.json"
  "fixtures/vault/m71_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m71-rollout-diagnostics-continuity-window-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_continuity_window_output.json" "$LATEST_DIR/rollout_diagnostics_continuity_window_output.json"
