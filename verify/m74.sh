...
{
  echo "# verify ${M} (rollout diagnostics automation rollback plan synthesis anchors)"
...
req=(
  ...
  "scripts/run-m74-rollout-diagnostics-rollback-plan-scenario.mjs"
  "fixtures/rollout/m74_scenario.json"
  "fixtures/rollout/m74_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m74-rollout-diagnostics-rollback-plan-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_rollback_plan_output.json" "$LATEST_DIR/rollout_diagnostics_rollback_plan_output.json"
