...
AUTHZ_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m143-market-settlement-bridge-scenario.mjs > "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/market_settlement_bridge_output.json" "$LATEST_DIR/market_settlement_bridge_output.json"
