...
{
  echo "# verify ${M} (rollout diagnostics automation execution attestation anchors)"
...
req=(
  ...
  "scripts/run-m68-rollout-diagnostics-execution-attestation-scenario.mjs"
  "fixtures/vault/m68_scenario.json"
  "fixtures/vault/m68_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m68-rollout-diagnostics-execution-attestation-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_execution_attestation_output.json" "$LATEST_DIR/rollout_diagnostics_execution_attestation_output.json"
