...
{
  echo "# verify ${M} (vault reconciliation export checkpoint retention hardening)"
  ...
  echo "$ AUTHZ_ENFORCE=1 SETTLEMENT_VAULT_EXPORT_CHECKPOINT_ENFORCE=1 SETTLEMENT_VAULT_EXPORT_CHECKPOINT_RETENTION_DAYS=1 OUT_DIR=$OUT_DIR node scripts/run-m54-vault-reconciliation-export-retention-scenario.mjs"
...
req=(
  ...
  "scripts/run-m54-vault-reconciliation-export-retention-scenario.mjs"
  "fixtures/vault/m54_scenario.json"
  "fixtures/vault/m54_expected.json"
...
AUTHZ_ENFORCE=1 SETTLEMENT_VAULT_EXPORT_CHECKPOINT_ENFORCE=1 SETTLEMENT_VAULT_EXPORT_CHECKPOINT_RETENTION_DAYS=1 OUT_DIR="$OUT_DIR" node scripts/run-m54-vault-reconciliation-export-retention-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/vault_reconciliation_export_retention_output.json" "$LATEST_DIR/vault_reconciliation_export_retention_output.json"
