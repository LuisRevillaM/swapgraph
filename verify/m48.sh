...
{
  echo "# verify ${M} (vault custody publication/read scaffold)"
  ...
  echo "$ OUT_DIR=$OUT_DIR node scripts/run-m48-vault-custody-publication-scenario.mjs"
...
req=(
  ...
  "scripts/run-m48-vault-custody-publication-scenario.mjs"
  "fixtures/vault/m48_scenario.json"
  "fixtures/vault/m48_expected.json"
...
OUT_DIR="$OUT_DIR" node scripts/run-m48-vault-custody-publication-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/vault_custody_publication_output.json" "$LATEST_DIR/vault_custody_publication_output.json"
