...
{
  echo "# verify ${M} (risk tier policy engine contract)"
  ...
  echo "$ AUTHZ_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m82-risk-tier-policy-engine-scenario.mjs"
...
req=(
  ...
  "scripts/run-m82-risk-tier-policy-engine-scenario.mjs"
  "fixtures/commercial/m82_scenario.json"
  "fixtures/commercial/m82_expected.json"
  "milestones/M82.yaml"
  "verify/m82.sh"
)
...
AUTHZ_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m82-risk-tier-policy-engine-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/risk_tier_policy_engine_output.json" "$LATEST_DIR/risk_tier_policy_engine_output.json"
