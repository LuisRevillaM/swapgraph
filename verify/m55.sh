...
{
  echo "# verify ${M} (vault export partner-program/commercial scaffold)"
  ...
  echo "$ AUTHZ_ENFORCE=1 SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE=1 OUT_DIR=$OUT_DIR node scripts/run-m55-vault-partner-program-scenario.mjs"
...
req=(
  ...
  "scripts/run-m55-vault-partner-program-scenario.mjs"
  "fixtures/vault/m55_scenario.json"
  "fixtures/vault/m55_expected.json"
...
AUTHZ_ENFORCE=1 SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE=1 OUT_DIR="$OUT_DIR" node scripts/run-m55-vault-partner-program-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/vault_partner_program_output.json" "$LATEST_DIR/vault_partner_program_output.json"
