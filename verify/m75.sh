...
{
  echo "# verify ${M} (rollout diagnostics automation simulation anchors)"
...
req=(
  ...
  "scripts/run-m75-rollout-diagnostics-simulation-scenario.mjs"
  "fixtures/rollout/m75_scenario.json"
  "fixtures/rollout/m75_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m75-rollout-diagnostics-simulation-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_simulation_output.json" "$LATEST_DIR/rollout_diagnostics_simulation_output.json"
