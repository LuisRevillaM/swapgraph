...
{
  echo "# verify ${M} (rollout diagnostics automation attestation continuity verification)"
...
req=(
  ...
  "scripts/run-m70-rollout-diagnostics-attestation-continuity-scenario.mjs"
  "fixtures/vault/m70_scenario.json"
  "fixtures/vault/m70_expected.json"
...
OUT_DIR="$OUT_DIR" \
node scripts/run-m70-rollout-diagnostics-attestation-continuity-scenario.mjs >> "$OUT_DIR/commands.log" 2>&1
...
cp "$OUT_DIR/rollout_diagnostics_attestation_continuity_output.json" "$LATEST_DIR/rollout_diagnostics_attestation_continuity_output.json"
