# M105 — LP vault inventory and reservation lifecycle contracts (PRD-only, implementation-ready)
...
## Scope
### A) LP inventory contracts (to define)
...
### B) LP reservation contracts (to define)
...
M105 is accepted when:
1. LP inventory/reservation operations are explicit,
2. lifecycle transitions and reservation invariants are explicit,
3. reconciliation export integrity boundaries are approved,
4. deterministic reason-code floor is approved.

## Acceptance (future implementation stage)
- `npm run verify:m105` exits 0.
- `node verify/runner.ts milestones/M105.yaml` returns `overall=true`.
