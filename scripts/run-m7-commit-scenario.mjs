...
// Load fixture intents and seed store.
...
const scenario = readJson(path.join(root, 'fixtures/commit/m7_scenario.json'));
...
for (const op of scenario.operations) {
  ...
  if (op.op === 'cycleProposals.accept') {
    ...
    const r = svc.accept({ actor: op.actor, idempotencyKey: op.idempotency_key, proposal, requestBody: req, occurredAt });
    const res = r.result;
  ...
  if (op.op === 'cycleProposals.decline') {
    ...
    const r = svc.decline({ actor: op.actor, idempotencyKey: op.idempotency_key, proposal, requestBody: req, occurredAt });
    const res = r.result;
  ...
  if (op.op === 'commits.get') {
    ...
    const res = svc.get({ actor: op.actor, commitId });

    if (res.ok) {
      const vres = validateFileSchema('CommitGetResponse.schema.json', res.body);
    ...
    opsResults.push({ op: op.op, commit_id: commitId, ok: res.ok, error_code: res.ok ? null : res.body.error.code, commit_phase: res.ok ? res.body.commit.phase : null });
    continue;
  }
...
const expectedPath = path.join(root, 'fixtures/commit/m7_expected.json');
...
writeFileSync(path.join(outDir, 'assertions.json'), JSON.stringify({ milestone: 'M7', status: 'pass' }, null, 2));
