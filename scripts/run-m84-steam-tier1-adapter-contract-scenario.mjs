...
const MILESTONE = 'M84';
const SCENARIO_FILE = 'fixtures/commercial/m84_scenario.json';
const EXPECTED_FILE = 'fixtures/commercial/m84_expected.json';
const OUTPUT_FILE = 'steam_tier1_adapter_contract_output.json';
...
const service = new SteamAdapterContractService({ store });
const operations = [];
...
for (const op of scenario.operations ?? []) {
  ...
  if (op.op === 'adapter.steam_tier1.contract.upsert') {
    ...
    const response = service.upsertTier1Contract({
      actor,
      auth: op.auth ?? {},
  ...
  if (op.op === 'adapter.steam_tier1.contract.get') {
    ...
    const response = service.getTier1Contract({ actor, auth: op.auth ?? {}, query });
    validateApiResponse(op.op, response);
  ...
  if (op.op === 'adapter.steam_tier1.preflight') {
    ...
    const response = service.preflightTier1Contract({
      actor,
      auth: op.auth ?? {},
      idempotencyKey: op.idempotency_key,
      request: op.request ?? {}
