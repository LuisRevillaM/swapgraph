...
if (process.env.AUTHZ_ENFORCE !== '1') {
  console.error('AUTHZ_ENFORCE must be 1 for M53 scenario');
  process.exit(2);
}
if (process.env.SETTLEMENT_VAULT_EXPORT_CHECKPOINT_ENFORCE !== '1') {
  console.error('SETTLEMENT_VAULT_EXPORT_CHECKPOINT_ENFORCE must be 1 for M53 scenario');
  process.exit(2);
}
...
const scenario = readJson(path.join(root, 'fixtures/vault/m53_scenario.json'));
const expected = readJson(path.join(root, 'fixtures/vault/m53_expected.json'));
...
for (const op of scenario.operations ?? []) {
  ...
  if (op.op.startsWith('vault.')) {
    ...
    if (op.op === 'vault.deposit') {
      ...
      throw new Error(`unsupported vault op in M53: ${op.op}`);
    }
  ...
  if (op.op.startsWith('settlement.')) {
    ...
    if (op.op === 'settlement.start') {
      ...
      throw new Error(`unsupported settlement op in M53: ${op.op}`);
    }
...
writeFileSync(path.join(outDir, 'vault_reconciliation_export_continuity_output.json'), JSON.stringify(out, null, 2));

assert.deepEqual(out, expected);

writeFileSync(path.join(outDir, 'assertions.json'), JSON.stringify({ milestone: 'M53', status: 'pass' }, null, 2));
