...
const MILESTONE = 'M141';
const SCENARIO_FILE = 'fixtures/release/m141_scenario.json';
const EXPECTED_FILE = 'fixtures/release/m141_expected.json';
const OUTPUT_FILE = 'market_threads_messages_output.json';
...
for (const op of scenario.operations ?? []) {
  ...
  if (op.op === 'marketThreads.create') {
    const request = clone(op.request ?? {});
    if (!op.skip_request_validation) validateApiRequest(op.op, request);
    const out = market.createThread({ actor, auth, idempotencyKey: op.idempotency_key, request });
    replayed = out.replayed;
    response = out.result;
  } else if (op.op === 'marketThreads.get') {
    response = market.getThread({ actor, auth, threadId: op.thread_id });
  } else if (op.op === 'marketThreads.list') {
    response = market.listThreads({ actor, auth, query: clone(op.query ?? {}) });
  } else if (op.op === 'marketThreadMessages.create') {
    const request = clone(op.request ?? {});
    if (!op.skip_request_validation) validateApiRequest(op.op, request);
    const out = market.createThreadMessage({ actor, auth, threadId: op.thread_id, idempotencyKey: op.idempotency_key, request });
    replayed = out.replayed;
    response = out.result;
  } else if (op.op === 'marketThreadMessages.list') {
    response = market.listThreadMessages({ actor, auth, threadId: op.thread_id, query: clone(op.query ?? {}) });
  } else {
  ...
  if (response.ok && response.body.thread) {
    rec.thread_id = response.body.thread.thread_id ?? null;
    rec.thread_status = response.body.thread.status ?? null;
  }

  if (response.ok && Array.isArray(response.body.threads)) {
    rec.total = Number(response.body.total ?? response.body.threads.length);
    rec.next_cursor = response.body.next_cursor ?? null;
    rec.first_thread_id = response.body.threads?.[0]?.thread_id ?? null;
  }

  if (response.ok && response.body.message) {
    rec.message_id = response.body.message.message_id ?? null;
    rec.message_type = response.body.message.message_type ?? null;
  }

  if (response.ok && Array.isArray(response.body.messages)) {
    rec.total = Number(response.body.total ?? response.body.messages.length);
    rec.next_cursor = response.body.next_cursor ?? null;
    rec.first_message_id = response.body.messages?.[0]?.message_id ?? null;
  }
...
const final = {
  market_threads_count: Object.keys(store.state.market_threads ?? {}).length,
  market_messages_count: Object.keys(store.state.market_messages ?? {}).length,
