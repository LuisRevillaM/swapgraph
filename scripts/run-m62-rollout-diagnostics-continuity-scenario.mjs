...
if (process.env.AUTHZ_ENFORCE !== '1') {
  console.error('AUTHZ_ENFORCE must be 1 for M62 scenario');
  process.exit(2);
}
if (process.env.SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE !== '1') {
  console.error('SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE must be 1 for M62 scenario');
  process.exit(2);
}
if (process.env.PARTNER_PROGRAM_ROLLOUT_POLICY_DIAGNOSTICS_EXPORT_CHECKPOINT_ENFORCE !== '1') {
  console.error('PARTNER_PROGRAM_ROLLOUT_POLICY_DIAGNOSTICS_EXPORT_CHECKPOINT_ENFORCE must be 1 for M62 scenario');
  process.exit(2);
}
...
const scenario = readJson(path.join(root, 'fixtures/vault/m62_scenario.json'));
const expected = readJson(path.join(root, 'fixtures/vault/m62_expected.json'));
...
for (const op of scenario.operations ?? []) {
  ...
  if (op.op === 'seed.vault_cycle') {
    const cycleId = String(op.cycle_id ?? 'cycle_m62');
    const partnerId = String(op.partner_id ?? actors.partner?.id ?? '');
...
writeFileSync(path.join(outDir, 'rollout_diagnostics_continuity_output.json'), JSON.stringify(out, null, 2));

assert.deepEqual(out, expected);

writeFileSync(path.join(outDir, 'assertions.json'), JSON.stringify({ milestone: 'M62', status: 'pass' }, null, 2));
