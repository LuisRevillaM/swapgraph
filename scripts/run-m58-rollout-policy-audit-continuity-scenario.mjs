...
if (process.env.AUTHZ_ENFORCE !== '1') {
  console.error('AUTHZ_ENFORCE must be 1 for M58 scenario');
  process.exit(2);
}
if (process.env.SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE !== '1') {
  console.error('SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE must be 1 for M58 scenario');
  process.exit(2);
}
if (process.env.PARTNER_PROGRAM_ROLLOUT_POLICY_EXPORT_CHECKPOINT_ENFORCE !== '1') {
  console.error('PARTNER_PROGRAM_ROLLOUT_POLICY_EXPORT_CHECKPOINT_ENFORCE must be 1 for M58 scenario');
  process.exit(2);
}
...
const scenario = readJson(path.join(root, 'fixtures/vault/m58_scenario.json'));
const expected = readJson(path.join(root, 'fixtures/vault/m58_expected.json'));
...
writeFileSync(path.join(outDir, 'rollout_policy_audit_continuity_output.json'), JSON.stringify(out, null, 2));

assert.deepEqual(out, expected);

writeFileSync(path.join(outDir, 'assertions.json'), JSON.stringify({ milestone: 'M58', status: 'pass' }, null, 2));
