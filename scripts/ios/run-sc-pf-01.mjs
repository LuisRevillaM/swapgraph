...
async function main() {
  const startup = await runSwiftFilter('PerformanceBudgetTests/testStartupBudgetForAppShellInitialization');
  const overall = startup.pass;
  ...
  const report = {
    ...
    checks: [
      {
        item: 'Cold startup budget for app shell initialization',
        filter: startup.filter,
        pass: startup.pass,
        exit_code: startup.exit_code,
        output_tail: startup.output_tail
      }
    ]
  };
