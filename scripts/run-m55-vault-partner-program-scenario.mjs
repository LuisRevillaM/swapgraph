...
if (process.env.AUTHZ_ENFORCE !== '1') {
  console.error('AUTHZ_ENFORCE must be 1 for M55 scenario');
  process.exit(2);
}
if (process.env.SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE !== '1') {
  console.error('SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE must be 1 for M55 scenario');
  process.exit(2);
}
...
const scenario = readJson(path.join(root, 'fixtures/vault/m55_scenario.json'));
const expected = readJson(path.join(root, 'fixtures/vault/m55_expected.json'));
...
for (const op of scenario.operations ?? []) {
  ...
  if (op.op.startsWith('vault.')) {
    ...
    if (op.op === 'vault.deposit') {
      ...
      throw new Error(`unsupported vault op in M55: ${op.op}`);
    }
  ...
  if (op.op.startsWith('settlement.')) {
    ...
    if (op.op === 'settlement.start') {
      ...
      throw new Error(`unsupported settlement op in M55: ${op.op}`);
    }
...
writeFileSync(path.join(outDir, 'vault_partner_program_output.json'), JSON.stringify(out, null, 2));

assert.deepEqual(out, expected);

writeFileSync(path.join(outDir, 'assertions.json'), JSON.stringify({ milestone: 'M55', status: 'pass' }, null, 2));
