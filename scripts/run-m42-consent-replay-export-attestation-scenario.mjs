...
if (process.env.AUTHZ_ENFORCE !== '1') {
  console.error('AUTHZ_ENFORCE must be 1 for M42 scenario');
  process.exit(2);
}
if (process.env.POLICY_CONSENT_TIER_ENFORCE !== '1') {
  console.error('POLICY_CONSENT_TIER_ENFORCE must be 1 for M42 scenario');
  process.exit(2);
}
if (process.env.POLICY_CONSENT_PROOF_BIND_ENFORCE !== '1') {
  console.error('POLICY_CONSENT_PROOF_BIND_ENFORCE must be 1 for M42 scenario');
  process.exit(2);
}
if (process.env.POLICY_CONSENT_PROOF_SIG_ENFORCE !== '1') {
  console.error('POLICY_CONSENT_PROOF_SIG_ENFORCE must be 1 for M42 scenario');
  process.exit(2);
}
if (process.env.POLICY_CONSENT_PROOF_REPLAY_ENFORCE !== '1') {
  console.error('POLICY_CONSENT_PROOF_REPLAY_ENFORCE must be 1 for M42 scenario');
  process.exit(2);
}
...
function withSignedConsentProof({ userConsent, subjectActor, intent, delegationId }) {
...
const scenario = readJson(path.join(root, 'fixtures/delegation/m42_scenario.json'));
const expected = readJson(path.join(root, 'fixtures/delegation/m42_expected.json'));
...
for (const op of scenario.operations ?? []) {
  ...
  if (op.op === 'policyAudit.export.verify_tampered_attestation') {
    ...
    const chain = String(tampered?.attestation?.chain_hash ?? '');
    const suffix = chain.endsWith('0') ? '1' : '0';
    tampered.attestation.chain_hash = `${chain.slice(0, Math.max(0, chain.length - 1))}${suffix}`;
...
writeFileSync(path.join(outDir, 'consent_replay_export_attestation_output.json'), JSON.stringify(out, null, 2));

assert.deepEqual(out, expected);

writeFileSync(path.join(outDir, 'assertions.json'), JSON.stringify({ milestone: 'M42', status: 'pass' }, null, 2));
