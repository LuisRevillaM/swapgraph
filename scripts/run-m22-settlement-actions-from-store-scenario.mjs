...
// ---- Scenario ----
const scenario = readJson(path.join(root, 'fixtures/settlement/m22_scenario.json'));
const expected = readJson(path.join(root, 'fixtures/settlement/m22_expected.json'));
...
store.state.tenancy.proposals[p3.id] = { partner_id: actors.partner_a.id };
...
for (const op of scenario.operations) {
  if (op.op === 'cycleProposals.accept') {
    ...
    operations.push({ op: op.op, cycle_id: cycleId, ok: res.ok, replayed: r.replayed, error_code: res.ok ? null : res.body.error.code, commit_phase: res.ok ? res.body.commit.phase : null });
    continue;
  }
  ...
  if (op.op === 'settlement.begin_execution') {
    ...
    const r = actionsSvc.beginExecution({ actor, cycleId, occurredAt: op.occurred_at });
  ...
  if (op.op === 'settlement.complete') {
    ...
    const r = actionsSvc.complete({ actor, cycleId, occurredAt: op.occurred_at });
...
writeFileSync(path.join(outDir, 'settlement_actions_output.json'), JSON.stringify(out, null, 2));

assert.deepEqual(out, expected);

writeFileSync(path.join(outDir, 'assertions.json'), JSON.stringify({ milestone: 'M22', status: 'pass' }, null, 2));
