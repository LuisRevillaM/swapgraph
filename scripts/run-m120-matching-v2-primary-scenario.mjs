...
const MILESTONE = 'M120';
const SCENARIO_FILE = 'fixtures/release/m120_scenario.json';
const EXPECTED_FILE = 'fixtures/release/m120_expected.json';
const OUTPUT_FILE = 'matching_v2_primary_output.json';
...
function createSeededStore({ seedState }) {
  const store = new JsonStateStore({ filePath: path.join(outDir, 'store_m120.json') });
  store.load();
...
function assertStepExpectations({ step, run, decision }) {
  ...
  if (expect.fallback_reason_code !== undefined) {
    assert.equal(decision?.fallback_reason_code ?? null, expect.fallback_reason_code, `fallback_reason_code mismatch for step=${step.label}`);
  }
  if (expect.rollback_reset_applied !== undefined) {
    assert.equal(Boolean(decision?.rollback_reset_applied), Boolean(expect.rollback_reset_applied), `rollback_reset_applied mismatch for step=${step.label}`);
  }
  ...
  if (expect.v2_error_code !== undefined) {
    assert.equal(decision?.v2?.error?.code ?? null, expect.v2_error_code, `v2_error_code mismatch for step=${step.label}`);
  }
...
for (const step of scenario.steps ?? []) {
  ...
  const out = withIsolatedMatcherEnv(
    ...
    () =>
      service.runMatching({
        ...
        idempotencyKey: `m120_${String(step?.label ?? 'step')}`,
  ...
  stepSummaries.push(
    canonicalize({
      ...
      fallback_reason_code: decision?.fallback_reason_code ?? null,
      canary_selected: Boolean(decision?.canary_selected),
      skipped_reason: decision?.skipped_reason ?? null,
      rollback_reset_applied: Boolean(decision?.rollback_reset_applied),
      rollback: {
