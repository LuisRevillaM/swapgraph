...
const MILESTONE = 'M85';
const SCENARIO_FILE = 'fixtures/integration/m85_scenario.json';
const EXPECTED_FILE = 'fixtures/integration/m85_expected.json';
const OUTPUT_FILE = 'steam_deposit_per_swap_live_proof_output.json';

if (process.env.INTEGRATION_ENABLED !== '1') {
  throw new Error('M85 integration gate requires INTEGRATION_ENABLED=1');
}
...
for (const op of scenario.operations ?? []) {
  ...
  if (op.op === 'adapter.steam_tier1.live_proof.deposit_per_swap.record') {
    if (!op.skip_request_validation) validateApiRequest(op.op, op.request ?? {});

    const response = withIntegrationOverride(op.integration_enabled_override, () => liveProofService.recordDepositPerSwapLiveProof({
      actor,
      auth: op.auth ?? {},
      idempotencyKey: op.idempotency_key,
      request: op.request ?? {}
    ...
    const rec = {
      ...
      steam_trade_offer_id: response.ok ? (response.body.proof?.steam_trade_offer_id ?? null) : null,
