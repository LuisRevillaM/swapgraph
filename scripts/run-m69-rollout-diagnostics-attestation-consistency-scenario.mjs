...
if (process.env.AUTHZ_ENFORCE !== '1') {
  console.error('AUTHZ_ENFORCE must be 1 for M69 scenario');
  process.exit(2);
}
if (process.env.SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE !== '1') {
  console.error('SETTLEMENT_VAULT_EXPORT_PARTNER_PROGRAM_ENFORCE must be 1 for M69 scenario');
  process.exit(2);
}
if (process.env.PARTNER_PROGRAM_ROLLOUT_POLICY_DIAGNOSTICS_EXPORT_CHECKPOINT_ENFORCE !== '1') {
  console.error('PARTNER_PROGRAM_ROLLOUT_POLICY_DIAGNOSTICS_EXPORT_CHECKPOINT_ENFORCE must be 1 for M69 scenario');
  process.exit(2);
}
...
const scenario = readJson(path.join(root, 'fixtures/vault/m69_scenario.json'));
const expected = readJson(path.join(root, 'fixtures/vault/m69_expected.json'));
...
for (const op of scenario.operations ?? []) {
  ...
  if (op.op === 'seed.vault_cycle') {
    const cycleId = String(op.cycle_id ?? 'cycle_m69');
    const partnerId = String(op.partner_id ?? actors.partner?.id ?? '');
  ...
  if (op.op === 'partnerProgram.vault_export.rollout_policy.diagnostics.export.verify_resigned_execution_attestation_tampered') {
    const payload = diagnosticsExportRefs[op.export_ref];
    if (!payload) throw new Error(`missing diagnostics export_ref: ${op.export_ref}`);
    ...
    const originalExpectedEffectHash = String(execution.expected_effect_hash ?? '');
    if (!/^[a-f0-9]{64}$/.test(originalExpectedEffectHash)) {
      throw new Error(`execution_attestation.expected_effect_hash is not a 64-hex hash for export_ref: ${op.export_ref}`);
    }

    const suffix = originalExpectedEffectHash.endsWith('0') ? '1' : '0';
    execution.expected_effect_hash = `${originalExpectedEffectHash.slice(0, Math.max(0, originalExpectedEffectHash.length - 1))}${suffix}`;
...
writeFileSync(path.join(outDir, 'rollout_diagnostics_attestation_consistency_output.json'), JSON.stringify(out, null, 2));

assert.deepEqual(out, expected);

writeFileSync(path.join(outDir, 'assertions.json'), JSON.stringify({ milestone: 'M69', status: 'pass' }, null, 2));
