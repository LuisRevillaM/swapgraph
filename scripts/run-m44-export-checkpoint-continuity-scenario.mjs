...
if (process.env.AUTHZ_ENFORCE !== '1') {
  console.error('AUTHZ_ENFORCE must be 1 for M44 scenario');
  process.exit(2);
}
if (process.env.POLICY_AUDIT_EXPORT_CHECKPOINT_ENFORCE !== '1') {
  console.error('POLICY_AUDIT_EXPORT_CHECKPOINT_ENFORCE must be 1 for M44 scenario');
  process.exit(2);
}
...
const scenario = readJson(path.join(root, 'fixtures/delegation/m44_scenario.json'));
const expected = readJson(path.join(root, 'fixtures/delegation/m44_expected.json'));
...
writeFileSync(path.join(outDir, 'export_checkpoint_continuity_output.json'), JSON.stringify(out, null, 2));

assert.deepEqual(out, expected);

writeFileSync(path.join(outDir, 'assertions.json'), JSON.stringify({ milestone: 'M44', status: 'pass' }, null, 2));
